...
class DoomNormalizeReward(gym.Wrapper):
    """
    Normalize discounted returns instead of individual immediate rewards.
    This is much more stable for PPO.
    """
    ...
    def step(self, action):
        ...
        self.return_rms.update(
            np.array([self.returns], dtype=np.float64)
        )

        normalized_reward = reward / np.sqrt(
        ...
        return obs, float(normalized_reward), done, info
  
class DoomRandomStart(gym.Wrapper):

    def __init__(self, env, max_turn_steps=5):
        super().__init__(env)
    ...
    def reset(self, **kwargs):
        ...
        n_steps = random.randint(0, self.max_turn_steps)

        for _ in range(n_steps):

            # Pick a VALID action index
            turn_action = random.randrange(n_actions)

            obs, _, done, _ = self.env.step(turn_action)
